...
const FuelLogModal: React.FC<FuelLogModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isNaN(odoNum) || odoNum < 0) {
      setError('Sila masukkan bacaan odometer semasa yang sah.');
      return;
    }
    if (isNaN(litersNum) || litersNum <= 0) {
      setError('Sila masukkan jumlah liter bahan api yang sah.');
      return;
    }
    if (isNaN(costNum) || costNum <= 0) {
      setError('Sila masukkan jumlah kos belian (RM) yang sah.');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex justify-center items-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col my-8 border border-slate-100 animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold text-slate-800">
                {logToEdit ? 'Kemaskini Log Bahan Api' : 'Daftar Log Bahan Api Baru'}
              </h2>
              <p className="text-xs text-slate-500">
                {logToEdit ? 'Ubah rekod belian minyak dan resit' : 'Kemasukan rekod isian minyak kenderaan'}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Tarikh Isian *</label>
              <input
                type="date"
                name="date"
                value={formData.date}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Odometer Semasa (KM) *</label>
              <input
                type="number"
                name="odometer"
                placeholder="Cth: 45200"
          ...
          <div className="p-4 bg-amber-50/50 border border-amber-200/70 rounded-xl space-y-3">
            <span className="text-[11px] font-bold text-amber-800 uppercase tracking-wider block">
              Butiran Pembelian Bahan Api
            </span>
            <div className="grid grid-cols-3 gap-2.5">
              <div>
                <label className="block text-[11px] font-bold text-slate-700 mb-1">Harga/Liter (RM)</label>
                <input
                  type="number"
                  step="0.01"
                  name="pricePerLiter"
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-700 mb-1">Kuantiti (Liter) *</label>
                <input
                  type="number"
                  step="0.01"
                  name="liters"
                  placeholder="Cth: 35.5"
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-700 mb-1">Jumlah Kos (RM) *</label>
                <input
                  type="number"
                  step="0.01"
                  name="cost"
                  placeholder="Cth: 72.80"
                  value={formData.cost}
                  onChange={handleChange}
                  required
            ...
            <p className="text-[11px] text-amber-800/80 italic text-center">
              * Jumlah Kos dikira secara automatik (Liter × Harga/Liter) dan boleh diubah mengikut resit fizikal.
            </p>
          </div>

          {/* Receipt Attachment */}
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">Lampiran Resit Pembelian</label>
            <div className="mt-1 flex items-center space-x-3">
              ...
              >
                ...
                {receiptFile || existingReceiptName ? 'Tukar Resit' : 'Muat Naik Resit'}
              </button>
          ...
          <div className="pt-3 border-t border-slate-100 flex justify-end space-x-2.5">
            ...
            >
              {logToEdit ? 'Simpan Perubahan' : 'Daftar Log Bahan Api'}
            </button>
          </div>
        </form>
      </div>
